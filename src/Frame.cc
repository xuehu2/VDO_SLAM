...
//Copy Constructor
    Frame::Frame(const Frame &frame)
            : mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
              ...
              mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), mThDepthObj(frame.mThDepthObj), N(frame.N),
              mvKeys(frame.mvKeys),
            ...
            // new added
              ...
              bObjStat(frame.bObjStat), vObjMod(frame.vObjMod), mvCorres(frame.mvCorres),
 ...
 */
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imFlow, const cv::Mat &maskSEM,
        ...
        // ------------------------------------------------------------------------------------------
        // ++++++++++++++++++++++++++++ New added for background features 背景的特征点 +++++++++++++++++++++++++++
        // ------------------------------------------------------------------------------------------
        ...
        // ---------------------------------------------------------------------------------------
        // ++++++++++++++++++++++++++++ New added for dense object features 目标的特征点 ++++++++++++++++++++++
        // ---------------------------------------------------------------------------------------

        // semi-dense features on objects 目标上的半稠密特征点信息
        int step = 4; // 4
    ...
    /**
     * 提取特征点，用mpORBextractorLeft or Right 提取特征点到mvKeys和mDescriptors
     * @param flag 左右标志位
     * @param im 图像
     */
