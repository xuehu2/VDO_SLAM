 ...
 *
 * Copyright (C) 2019-2020 Jun Zhang <jun doc zhang2 at anu dot edu doc au> (The
 *Australian National University) For more information see
...
namespace VDO_SLAM {
    ...
    Tracking::Tracking(System *pSys, Map *pMap, const string &strSettingPath,
        ...
        if (sensor == System::STEREO || sensor == System::RGBD) {
            ...
            cout << "- depth threshold (background/object): " << mThDepth << "/"
                 << mThDepthObj << endl;
        }
        ...
        cout << "- max tracking points: "
             << "(1) background: " << nMaxTrackPointBG
             << " (2) object: " << nMaxTrackPointOBJ << endl;

        fSFMgThres = fSettings["SFMgThres"];
        fSFDsThres = fSettings["SFDsThres"];
        cout << "- scene flow paras: "
             << "(1) magnitude: " << fSFMgThres << " (2) percentage: " << fSFDsThres
             << endl;

        nWINDOW_SIZE = fSettings["WINDOW_SIZE"];
        nOVERLAP_SIZE = fSettings["OVERLAP_SIZE"];
        cout << "- local batch paras: "
             << "(1) window: " << nWINDOW_SIZE << " (2) overlap: " << nOVERLAP_SIZE
             << endl;
 ...
 * @return cv::Mat 相机的位姿
 */
    cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, cv::Mat &imD,
                                    const cv::Mat &imFlow, const cv::Mat &maskSEM,
                                    const cv::Mat &mTcw_gt,
                                    const vector<vector<float>> &vObjPose_gt,
...
//                        imD.at<float>(i, j) = mbf / (imD.at<float>(i, j) / mDepthMapFactor);
        ...
        // Initialize timing vector (Output) 计算耗时
        ...
        if (mState != NO_IMAGES_YET) {
            ...
            for (int i = 0; i < mCurrentFrame.mvObjKeys.size(); ++i) {
                ...
                if (u < (mImGray.cols - 1) && u > 0 && v < (mImGray.rows - 1) && v > 0 &&
                    imDepth.at<float>(v, u) < mThDepthObj &&
                    imDepth.at<float>(v, u) > 0) {
            ...
            // Feature Distribution 2", img_show); cv::waitKey(0);
        ...
        if (timestamp != 0 && bFrame2Frame == true) {
            ...
            //         continue;
            //     cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,0,255),
            //     1); // red
            // }
            for (int i = 0; i < TemperalMatch_subset.size(); i = i + 1) {
                ...
                if (KeyPoints_tmp[0].pt.x >= (mImGray.cols - 1) ||
                    KeyPoints_tmp[0].pt.x <= 0 ||
                    KeyPoints_tmp[0].pt.y >= (mImGray.rows - 1) ||
                    KeyPoints_tmp[0].pt.y <= 0)
                    continue;
                if (maskSEM.at<int>(KeyPoints_tmp[0].pt.y, KeyPoints_tmp[0].pt.x) != 0)
                    continue;
            ...
            for (int i = 0; i < mCurrentFrame.vObjLabel.size(); ++i) {
                ...
                switch (l) {
                    case 0:
                        ...
                        break;
                    case 1:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(128, 0, 128),
                                          1); // 255, 165, 0
                        break;
                    case 2:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255, 255, 0),
                                          1);
                        break;
                    case 3:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0, 255, 0),
                                          1); // 255,255,0
                        break;
                    case 4:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255, 0, 0),
                                          1); // 255,192,203
                        break;
                    case 5:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0, 255, 255),
                                          1);
                        break;
                    case 6:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(128, 0, 128),
                        ...
                        break;
                    case 7:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB,
                                          cv::Scalar(255, 255, 255), 1);
                        break;
                    case 8:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB,
                                          cv::Scalar(255, 228, 196), 1);
                        break;
                    case 9:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB,
                                          cv::Scalar(180, 105, 255), 1);
                        break;
                    case 10:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(165, 42, 42),
                                          1);
                        break;
                    case 11:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(35, 142, 107),
                                          1);
                        break;
                    case 12:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(45, 82, 160),
                                          1);
                        break;
                    case 13:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0, 0, 255),
                        ...
                        break;
                    case 14:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255, 165, 0),
                                          1);
                        break;
                    case 15:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0, 255, 0),
                    ...
                    case 25:
                        ...
                        break;
                    case 41:
                        cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(60, 20, 220),
                                          1);
                        break;
                }
            }
        ...
        if (timestamp != 0 && bFrame2Frame == true && mTestData == KITTI) {
            ...
            for (int i = 0; i < mCurrentFrame.vObjBoxID.size(); ++i) {
                ...
                cv::Point pt1(vObjPose_gt[mCurrentFrame.vObjBoxID[i]][2],
                              vObjPose_gt[mCurrentFrame.vObjBoxID[i]][3]);
                cv::Point pt2(vObjPose_gt[mCurrentFrame.vObjBoxID[i]][4],
                              vObjPose_gt[mCurrentFrame.vObjBoxID[i]][5]);
                ...
                cv::putText(mImBGR, output_est, cv::Point(pt1.x, pt1.y - 10),
                            cv::FONT_HERSHEY_DUPLEX, 0.9, CV_RGB(0, 255, 0),
                            2); // CV_RGB(255,140,0)
                // cv::putText(mImBGR, output_gt, cv::Point(pt1.x, pt1.y-32),
                // cv::FONT_HERSHEY_DUPLEX, 0.7, CV_RGB(255, 0, 0), 2);
            }
        ...
        if (mTestData == KITTI) {
            ...
            cv::rectangle(imTraj, cv::Point(x, y), cv::Point(x + 5, y + 5),
                          cv::Scalar(0, 0, 255), 1);
            cv::rectangle(imTraj, cv::Point(10, 30), cv::Point(550, 60),
                          CV_RGB(0, 0, 0), CV_FILLED);
            cv::putText(imTraj, "Camera Trajectory (RED SQUARE)", cv::Point(10, 30),
                        cv::FONT_HERSHEY_COMPLEX, 0.6, CV_RGB(255, 255, 255), 1);
            char text[100];
            sprintf(text, "x = %02fm y = %02fm z = %02fm", CamPos.at<float>(0, 3),
                    CamPos.at<float>(1, 3), CamPos.at<float>(2, 3));
            cv::putText(imTraj, text, cv::Point(10, 50), cv::FONT_HERSHEY_COMPLEX, 0.6,
                        cv::Scalar::all(255), 1);
            cv::putText(imTraj, "Object Trajectories (COLORED CIRCLES)",
                        cv::Point(10, 70), cv::FONT_HERSHEY_COMPLEX, 0.6,
                        CV_RGB(255, 255, 255), 1);

            for (int i = 0; i < mCurrentFrame.vObjCentre3D.size(); ++i) {
                if (mCurrentFrame.vObjCentre3D[i].at<float>(0, 0) == 0 &&
                    mCurrentFrame.vObjCentre3D[i].at<float>(0, 2) == 0)
                    continue;
                int x =
                        int(mCurrentFrame.vObjCentre3D[i].at<float>(0, 0) * scale) + sta_x;
                int y =
                        int(mCurrentFrame.vObjCentre3D[i].at<float>(0, 2) * scale) + sta_y;
                // int l = mCurrentFrame.nSemPosition[i];
                int l = mCurrentFrame.nModLabel[i];
                switch (l) {
                    case 1:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(128, 0, 128),
                                   thic); // orange
                        break;
                    case 2:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(0, 255, 255),
                                   thic); // green
                        break;
                    case 3:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(0, 255, 0),
                                   thic); // yellow
                        break;
                    case 4:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(0, 0, 255),
                                   thic); // pink
                        break;
                    case 5:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(255, 255, 0),
                                   thic); // cyan (yellow green 47,255,173)
                        break;
                    case 6:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(128, 0, 128),
                                   thic); // purple
                        break;
                    case 7:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(255, 255, 255),
                                   thic); // white
                        break;
                    case 8:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(196, 228, 255),
                                   thic); // bisque
                        break;
                    case 9:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(180, 105, 255),
                                   thic); // blue
                        break;
                    case 10:
                        cv::circle(imTraj, cv::Point(x, y), radi, CV_RGB(42, 42, 165),
                                   thic); // brown
                        break;
                    case 11:
        ...
        if (timestamp != 0 && bFrame2Frame == true && mTestData == OMD) {
            PlotMetricError(mpMap->vmCameraPose, mpMap->vmRigidMotion,
                            mpMap->vmObjPosePre, mpMap->vmCameraPose_GT,
        ...
        //                 TemperalMatches, img_matches, cv::Scalar::all(-1),
        //                 cv::Scalar::all(-1), vector<char>(),
        //                 cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
        //     cv::resize(img_matches, img_matches, cv::Size(img_matches.cols/1.0,
        //     img_matches.rows/1.0)); cv::namedWindow("temperal matches",
        //     cv::WINDOW_NORMAL); cv::imshow("temperal matches", img_matches);
        //     cv::waitKey(0);
        // }
    ...
    void Tracking::Track() {
        ...
        if (mState == NOT_INITIALIZED) {
            ...
            if (bJoint)  // 预先设定为True
                ...
                Optimizer::PoseOptimizationNew(&mCurrentFrame, &mLastFrame,
                                               TemperalMatch_subset);
            // cout << "pose after update: " << endl << mCurrentFrame.mTcw << endl;
            e_1_2 = clock();
            cam_pos_time = (double) (e_1_1 - s_1_1) / CLOCKS_PER_SEC * 1000 +
                           (double) (e_1_2 - s_1_2) / CLOCKS_PER_SEC * 1000;
            all_timing[1] = cam_pos_time;
            // cout << "camera pose estimation time: " << cam_pos_time << endl;

            // Update motion model  更新运动模型 和ORB2一致
            if (!mLastFrame.mTcw.empty()) {// 如果上一帧的位姿已知
            ...
            cv::Mat T_lc_inv =
                    mCurrentFrame.mTcw * Converter::toInvMatrix(mLastFrame.mTcw);
            cv::Mat T_lc_gt =
                    mLastFrame.mTcw_gt * Converter::toInvMatrix(mCurrentFrame.mTcw_gt);
            cv::Mat RePoEr_cam = T_lc_inv * T_lc_gt;

            float t_rpe_cam =
                    std::sqrt(RePoEr_cam.at<float>(0, 3) * RePoEr_cam.at<float>(0, 3) +
                              RePoEr_cam.at<float>(1, 3) * RePoEr_cam.at<float>(1, 3) +
                              RePoEr_cam.at<float>(2, 3) * RePoEr_cam.at<float>(2, 3));
            float trace_rpe_cam = 0;
            for (int i = 0; i < 3; ++i) {
                if (RePoEr_cam.at<float>(i, i) > 1.0)
                    trace_rpe_cam =
                            trace_rpe_cam + 1.0 - (RePoEr_cam.at<float>(i, i) - 1.0);
                else
            ...
            cout << "the relative pose error of estimated camera pose, "
                 << "t: " << t_rpe_cam << " R: " << r_rpe_cam << endl;
            ...
            // }
            // cv::drawKeypoints(mImGray, PickKeys, mImGray, cv::Scalar::all(-1),
            // cv::DrawMatchesFlags::DEFAULT); cv::imshow("KeyPoints and Grid on
            ...
            for (int i = 0; i < ObjIdNew.size(); ++i) {
                cout << endl
                     << "Processing Object No.[" << mCurrentFrame.nModLabel[i]
                     << "]:" << endl;
                ...
                if (!bCheckGT1 || !bCheckGT2) {
                    cout << "Found a detected object with no ground truth motion! ! !"
                         << endl;
                    ...
                    mCurrentFrame.vObjCentre3D[i] =
                            (cv::Mat_<float>(3, 1) << 0.f, 0.f, 0.f);
                ...
                // cout << "ground truth motion of object No. " <<
                // mCurrentFrame.nSemPosition[i] << " :" << endl; cout << H_p_c << endl;
                ...
                // cv::Mat H_tmp =
                // Converter::toInvMatrix(mCurrentFrame.mTcw_gt)*mCurrentFrame.mInitModel;
                ...
                // cout << "number of pick points: " << ObjIdTest_in.size() << "/" <<
                // ObjIdTest.size() << "/" << mCurrentFrame.mvObjKeys.size() << endl;
                ...
                // cv::Mat img_matches;
                // drawMatches(mImGrayLast, PreKeys, mImGray, CurKeys,
                //             TMes, img_matches, cv::Scalar::all(-1),
                //             cv::Scalar::all(-1), vector<char>(),
                //             cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
                // cv::resize(img_matches, img_matches, cv::Size(img_matches.cols/1.0,
                // img_matches.rows/1.0)); cv::namedWindow("temperal matches",
                ...
                if (bJoint) {
                    cv::Mat Obj_X_tmp = Optimizer::PoseOptimizationFlow2(
                            &mCurrentFrame, &mLastFrame, ObjIdTest_in, InlierID);
                    mCurrentFrame.vObjMod[i] =
                            Converter::toInvMatrix(mCurrentFrame.mTcw) * Obj_X_tmp;
                } else
                    mCurrentFrame.vObjMod[i] = Optimizer::PoseOptimizationObjMot(
                            &mCurrentFrame, &mLastFrame, ObjIdTest_in, InlierID);
                e_3_2 = clock();
                t_con = t_con + 1;
                obj_mot_time = obj_mot_time +
                               (double) (e_3_1 - s_3_1) / CLOCKS_PER_SEC * 1000 +
                               (double) (e_3_2 - s_3_2) / CLOCKS_PER_SEC * 1000;

                mCurrentFrame.vnObjInlierID[i] = InlierID;

                // cout << "computed motion of object No. " <<
                // mCurrentFrame.nSemPosition[i] << " :" << endl; cout <<
                // mCurrentFrame.vObjMod[i] << endl;

                // ***********************************************************************************************

                // // ***** get the ground truth object speed here ***** (use version 1
                // here)
                cv::Mat sp_gt_v, sp_gt_v2;
                sp_gt_v =
                        H_p_c.rowRange(0, 3).col(3) -
                        (cv::Mat::eye(3, 3, CV_32F) - H_p_c.rowRange(0, 3).colRange(0, 3)) *
                        ObjCentre3D_pre; // L_w_p.rowRange(0,3).col(3) or ObjCentre3D_pre
                sp_gt_v2 = L_w_p.rowRange(0, 3).col(3) - L_w_c.rowRange(0, 3).col(3);
                float sp_gt_norm =
                        std::sqrt(sp_gt_v.at<float>(0) * sp_gt_v.at<float>(0) +
                                  sp_gt_v.at<float>(1) * sp_gt_v.at<float>(1) +
                                  sp_gt_v.at<float>(2) * sp_gt_v.at<float>(2)) *
                        36;
                // float sp_gt_norm2 = std::sqrt(
                // sp_gt_v2.at<float>(0)*sp_gt_v2.at<float>(0) +
                // sp_gt_v2.at<float>(1)*sp_gt_v2.at<float>(1) +
                // sp_gt_v2.at<float>(2)*sp_gt_v2.at<float>(2) )*36;
                mCurrentFrame.vObjSpeed_gt[i] = sp_gt_norm;

                // // ***** calculate the estimated object speed *****
                cv::Mat sp_est_v;
                sp_est_v = mCurrentFrame.vObjMod[i].rowRange(0, 3).col(3) -
                           (cv::Mat::eye(3, 3, CV_32F) -
                            mCurrentFrame.vObjMod[i].rowRange(0, 3).colRange(0, 3)) *
                           ObjCentre3D_pre;
                float sp_est_norm =
                        std::sqrt(sp_est_v.at<float>(0) * sp_est_v.at<float>(0) +
                                  sp_est_v.at<float>(1) * sp_est_v.at<float>(1) +
                                  sp_est_v.at<float>(2) * sp_est_v.at<float>(2)) *
                        36;

                cout << "estimated and ground truth object speed: " << sp_est_norm
                     << "km/h " << sp_gt_norm << "km/h " << endl;
                ...
                // cv::Mat H_p_c_body_est_inv =
                // Converter::toInvMatrix(mCurrentFrame.vObjMod[i]); cv::Mat RePoEr =
                // H_p_c_body_est_inv*H_p_c_body;

                float t_rpe = std::sqrt(RePoEr.at<float>(0, 3) * RePoEr.at<float>(0, 3) +
                                        RePoEr.at<float>(1, 3) * RePoEr.at<float>(1, 3) +
                                        RePoEr.at<float>(2, 3) * RePoEr.at<float>(2, 3));
                float trace_rpe = 0;
                for (int i = 0; i < 3; ++i) {
                ...
                cout << "the relative pose error of the object, "
                     << "t: " << t_rpe << " R: " << r_rpe << endl;
            ...
            mvKeysLastFrame =
                    mLastFrame.mvStatKeys; // new added (1st Dec)  mvStatKeys <-> mvKeys
            ...
            mLastFrame.vSemObjLabel =
                    mCurrentFrame.vSemObjLabel; // new added Nov 19 2019

            mLastFrame.mvStatKeys =
                    mCurrentFrame.mvStatKeysTmp; // new added Jul 30 2019
            mLastFrame.mvStatDepth =
                    mCurrentFrame.mvStatDepthTmp; // new added Jul 30 2019
            ...
            mpMap->vp3DPointSta.push_back(
                    mCurrentFrame.mvStat3DPointTmp); // (new added Dec 12 2019)
            mpMap->vnAssoSta.push_back(
                    mCurrentFrame.nStaInlierID); // (new added Nov 14 2019)

            // (2) detected dynamic object features, corresponding depth and
            // associations
            mpMap->vpFeatDyn.push_back(
                    mCurrentFrame.mvObjKeys); // (new added Nov 20 2019)
            mpMap->vfDepDyn.push_back(
                    mCurrentFrame.mvObjDepth); // (new added Nov 20 2019)
            mpMap->vp3DPointDyn.push_back(
                    mCurrentFrame.mvObj3DPoint); // (new added Dec 12 2019)
            mpMap->vnAssoDyn.push_back(
                    mCurrentFrame.nDynInlierID); // (new added Nov 20 2019)
            mpMap->vnFeatLabel.push_back(
                    mCurrentFrame.vObjLabel); // (new added Nov 20 2019)
            ...
            // (6) Rigid motions and label, including camera (label=0) and objects
            // (label>0)
            ...
            if (max_id > 1)
                mpMap->vnObjTraTime = GetObjTrackTime(mpMap->vnRMLabel, mpMap->vnSMLabel,
                                                      mpMap->vnSMLabelGT);
        ...
        // =================================================================================================
        // ============== Partial batch optimize on all the measurements (local
        // optimization) ==============
        // =================================================================================================

        bLocalBatch = true;
        if ((f_id - nOVERLAP_SIZE + 1) % (nWINDOW_SIZE - nOVERLAP_SIZE) == 0 &&
            f_id >= nWINDOW_SIZE - 1 && bLocalBatch) {
            cout << "-------------------------------------------" << endl;
        ...
        // =================================================================================================
        // ============== Full batch optimize on all the measurements (global
        // optimization) ================
        // =================================================================================================
        ...
        if (f_id == StopFrame) // bFrame2Frame f_id>=2
        {
            ...
            GetMetricError(mpMap->vmCameraPose, mpMap->vmRigidMotion,
                           mpMap->vmObjPosePre, mpMap->vmCameraPose_GT,
                           mpMap->vmRigidMotion_GT, mpMap->vbObjStat);
            // GetVelocityError(mpMap->vmRigidMotion, mpMap->vp3DPointDyn,
            // mpMap->vnFeatLabel,
            //                  mpMap->vnRMLabel, mpMap->vfAllSpeed_GT,
            //                  mpMap->vnAssoDyn, mpMap->vbObjStat);
            ...
            if (bGlobalBatch && mTestData == KITTI) {
                ...
                GetMetricError(mpMap->vmCameraPose_RF, mpMap->vmRigidMotion_RF,
                               mpMap->vmObjPosePre, mpMap->vmCameraPose_GT,
                               mpMap->vmRigidMotion_GT, mpMap->vbObjStat);
                // GetVelocityError(mpMap->vmRigidMotion_RF, mpMap->vp3DPointDyn,
                // mpMap->vnFeatLabel,
                //                  mpMap->vnRMLabel, mpMap->vfAllSpeed_GT,
                //                  mpMap->vnAssoDyn, mpMap->vbObjStat);
            }
 ...
 */
    void Tracking::TrackBack() {
        ...
        if (mState == NOT_INITIALIZED) {
            ...
            cout << "---计算相机位姿---" << endl;
            cout << "" << endl;
            for (int i = 0; i < mCurrentFrame.N_s; i++) { ;//todo
            }
    ...
    void Tracking::Initialization() {
        ...
        // initialize the 3d points
        {
            ...
            mCurrentFrame.mvObj3DPoint = mvObj3DPointTmp;
            // cout << "see the size 1: " << mCurrentFrame.mvStatKeysTmp.size() << " "
            // << mCurrentFrame.mvSift3DPoint.size() << endl; cout << "see the size 2: "
            // << mCurrentFrame.mvObjKeys.size() << " " <<
            // mCurrentFrame.mvObj3DPoint.size() << endl;
        }

        // (1) save detected static features and corresponding depth
        mpMap->vpFeatSta.push_back(
                mCurrentFrame.mvStatKeysTmp); // modified Nov 14 2019
        mpMap->vfDepSta.push_back(
                mCurrentFrame.mvStatDepthTmp); // modified Nov 14 2019
        mpMap->vp3DPointSta.push_back(
                mCurrentFrame.mvStat3DPointTmp); // modified Dec 17 2019
        ...
        mpMap->vp3DPointDyn.push_back(
                mCurrentFrame.mvObj3DPoint); // modified Dec 17 2019
        // (3) save camera pose
        ...
        // Converter::toInvMatrix(mOriginInv)*mCurrentFrame.mTcw_gt; cout << "mTcw_gt:
        // " << mCurrentFrame.mTcw_gt << endl; bFirstFrame = false; cout << "current
        // pose: " << endl << mCurrentFrame.mTcw_gt << endl; cout << "current pose
        // inverse: " << endl << mOriginInv << endl;
 ...
 */
    void Tracking::GetSceneFlowObj() {
        ...
        // Initialization 初始化
        int N = mCurrentFrame.mvObjKeys.size(); //当前帧目标上的关键点数量
        ...
        // Tracking::DrawSparseFlowBirdeye(pts_p3d, pts_vel,
        // Converter::toInvMatrix(mLastFrame.mTcw), viz_props, img_sparse_flow_3d);
    ...
    std::vector<std::vector<int>> Tracking::DynObjTracking() {
        ...
        for (int i = 0; i < Posi.size(); ++i) {
            ...
            for (int j = 0; j < Posi[i].size(); ++j) {
                ...
                if (v < shrin_thr_row || v > (mImGray.rows - shrin_thr_row) ||
                    u < shrin_thr_col || u > (mImGray.cols - shrin_thr_col))
                    count = count + 1;
            }
            if (count / Posi[i].size() > count_thres) {
                // cout << "Most part of this object is on the image boundary......" <<
                // endl;
        ...
        for (int i = 0; i < ObjId.size(); ++i) {
            ...
            float obj_center_depth = 0, sf_min = 100, sf_max = 0, sf_mean = 0,
                    sf_count = 0;
            std::vector<int> sf_range(10, 0);
            for (int j = 0; j < ObjId[i].size(); ++j) {
                obj_center_depth =
                        obj_center_depth + mCurrentFrame.mvObjDepth[ObjId[i][j]];
                // const float sf_norm = cv::norm(mCurrentFrame.vFlow_3d[ObjId[i][j]]);
                float sf_norm = std::sqrt(mCurrentFrame.vFlow_3d[ObjId[i][j]].x *
                                          mCurrentFrame.vFlow_3d[ObjId[i][j]].x +
                                          mCurrentFrame.vFlow_3d[ObjId[i][j]].z *
                                          mCurrentFrame.vFlow_3d[ObjId[i][j]].z);
            ...
            if (sf_count / ObjId[i].size() > fSFDsThres) {
                ...
                continue;
            } else if (obj_center_depth / ObjId[i].size() > mThDepthObj ||
                       ObjId[i].size() < 150) {
                obj_dis_tres[i] = -1;
                // cout << "object " << sem_posi[i] <<" is too far away or too small! " <<
                // obj_center_depth/ObjId[i].size() << endl; label this object as far away
                // object
                for (int k = 0; k < ObjId[i].size(); ++k)
 ...
 */
    cv::Mat Tracking::GetInitModelCam(const std::vector<int> &MatchId,
        ...
        for (int i = 0; i < N; ++i) {
            const cv::Mat x3D =
                    (cv::Mat_<float>(3, 1) << pre_3d[i].x, pre_3d[i].y, pre_3d[i].z);
            const cv::Mat x3D_c = MotionModel.rowRange(0, 3).colRange(0, 3) * x3D +
                                  MotionModel.rowRange(0, 3).col(3);
        ...
        // cout << "Inlier Compare: " << "(1)AP3P RANSAC: " << inliers.rows << "
        ...
        if (inliers.rows > MM_inlier.size()) {
            ...
            // cout << "(Camera) AP3P+RanSac inliers/total number: " << inliers.rows <<
            // "/" << MatchId.size() << endl;
        } else {
            output = MotionModel;
            ...
            // cout << "(Camera) Motion Model inliers/total number: " <<
            // MM_inlier.size() << "/" << MatchId.size() << endl;
        }

        return output;
    }

    cv::Mat Tracking::GetInitModelObj(const std::vector<int> &ObjId,
                                      std::vector<int> &ObjId_sub,
                                      const int objid) {
        ...
        if (PreObjID != -1) {
            ...
            if (inliers.rows > MM_inlier.size()) {
                ...
                // cout << "(Object) AP3P+RanSac inliers/total number: " << inliers.rows
                // << "/" << ObjId.size() << endl;
            } else {
                output = MotionModel;
                ...
                // cout << "(Object) Motion Model inliers/total number: " <<
                // MM_inlier.size() << "/" << ObjId.size() << endl;
            }
        } else {
            // save the inliers IDs
            output = Mod;
            ...
            // cout << "(Object) AP3P+RanSac [No MM] inliers/total number: " <<
            // inliers.rows << "/" << ObjId.size() << endl;
        }
    ...
    void Tracking::DrawLine(cv::KeyPoint &keys, cv::Point2f &flow,
        ...
        if ((cv_p1.x < 0) && (cv_p1.y < 0) && (cv_p1.x > ref_image.cols) &&
            (cv_p1.y > ref_image.rows))
            p1_in_bounds = false;

        if ((cv_p2.x < 0) && (cv_p2.y < 0) && (cv_p2.x > ref_image.cols) &&
            (cv_p2.y > ref_image.rows))
            p2_in_bounds = false;

        // Draw line, but only if both end-points project into the image!
        if (p1_in_bounds || p2_in_bounds) { // This is correct. Won't draw only if
            // both lines are out of bounds.
            // Draw line
            auto p1_offs = offset + cv_p1;
            auto p2_offs = offset + cv_p2;
            if (cv::clipLine(cv::Size(ref_image.cols, ref_image.rows), p1_offs,
                             p2_offs)) {
    ...
    void Tracking::DrawTransparentSquare(cv::Point center, cv::Vec3b color,
        ...
        for (int i = -radius; i < radius; i++) {
            for (int j = -radius; j < radius; j++) {
                ...
                if (coord_x > 0 && coord_y > 0 && coord_x < ref_image.cols &&
                    coord_y < ref_image.rows) {
    ...
    void Tracking::DrawGridBirdeye(double res_x, double res_z,
                                   const BirdEyeVizProperties &viz_props,
                                   cv::Mat &ref_image) {
        ...
        // Draw vertical lines
        for (double i = viz_props.birdeye_left_plane_;
             i < viz_props.birdeye_right_plane_; i += res_x) {
            double x_1 = i;
            double y_1 = 0;
            double x_2 = i;
    ...
    void Tracking::DrawSparseFlowBirdeye(const std::vector<Eigen::Vector3d> &pts,
        ...
        flip_mat << viz_props.birdeye_scale_factor_ * 1.0, 0, 0,
                viz_props.birdeye_scale_factor_ * 1.0;
        Eigen::Matrix2d world_to_cam_mat;
        const Eigen::Matrix4d &ref_to_rt_inv = Converter::toMatrix4d(camera);
        world_to_cam_mat << ref_to_rt_inv(0, 0), ref_to_rt_inv(2, 0),
                ref_to_rt_inv(0, 2), ref_to_rt_inv(2, 2);
        ...
        ref_image = cv::Mat(
                viz_props.birdeye_scale_factor_ * viz_props.birdeye_far_plane_,
                (-viz_props.birdeye_left_plane_ + viz_props.birdeye_right_plane_) *
                viz_props.birdeye_scale_factor_,
                CV_32FC3);
        ...
        for (int i = 0; i < pts.size(); i++) {
            ...
            if (x_1 > viz_props.birdeye_left_plane_ &&
                x_2 > viz_props.birdeye_left_plane_ &&
                x_1 < viz_props.birdeye_right_plane_ &&
                x_2 < viz_props.birdeye_right_plane_ && z_1 > 0 && z_2 > 0 &&
                z_1 < viz_props.birdeye_far_plane_ &&
                z_2 < viz_props.birdeye_far_plane_) {

                TransformPointToScaledFrustum(x_1, z_1,
                                              viz_props); // velocity[0], velocity[1]);
                TransformPointToScaledFrustum(x_2, z_2,
                                              viz_props); // velocity[0], velocity[1]);

                cv::arrowedLine(ref_image, cv::Point(x_1, z_1), cv::Point(x_2, z_2),
                                cv::Scalar(1.0, 0.0, 0.0), 1);
                cv::circle(ref_image, cv::Point(x_1, z_1), 3.0, cv::Scalar(0.0, 0.0, 1.0),
                           -1.0);
            }
        }
        ...
        // cv::putText(ref_image, "X", cv::Point(ref_image.cols/2+arrow_len+10,
        // offset_y+10), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(1.0, 0, 0));
        // cv::putText(ref_image, "Z", cv::Point(ref_image.cols/2+10,
        // offset_y+arrow_len), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8,
        // cv::Scalar(0.0, 1.0, 0));

        // Flip image, because it is more intuitive to have ref. point at the bottom
        // of the image
        cv::Mat dst;
    ...
    void Tracking::TransformPointToScaledFrustum(
            double &pose_x, double &pose_z, const BirdEyeVizProperties &viz_props) {
 ...
 */
    cv::Mat Tracking::ObjPoseParsingOX(const std::vector<float> &vObjPose_gt) {
        ...
        const float angle = std::sqrt(vObjPose_gt[5] * vObjPose_gt[5] +
                                      vObjPose_gt[6] * vObjPose_gt[6] +
                                      vObjPose_gt[7] * vObjPose_gt[7]);
    ...
    void Tracking::StackObjInfo(std::vector<cv::KeyPoint> &FeatDynObj,
        ...
        for (int i = 0; i < mCurrentFrame.vnObjID.size(); ++i) {
            for (int j = 0; j < mCurrentFrame.vnObjID[i].size(); ++j) {
                ...
                FeatDynObj.push_back(
                        mCurrentFrame.mvObjKeys[mCurrentFrame.vnObjID[i][j]]);
                DepDynObj.push_back(mLastFrame.mvObjDepth[mCurrentFrame.vnObjID[i][j]]);
                DepDynObj.push_back(
                        mCurrentFrame.mvObjDepth[mCurrentFrame.vnObjID[i][j]]);
                FeatLabObj.push_back(
                        mCurrentFrame.vObjLabel[mCurrentFrame.vnObjID[i][j]]);
            }
        }
    }
    ...
    std::vector<std::vector<std::pair<int, int>>> Tracking::GetStaticTrack() {
                ...
                if (i == 0) {
                    ...
                    if (TemporalMatch[i][j] != -1) {
                        ...
                        if (TrackCheck_pre[TemporalMatch[i][j]] != -1) {
                            TrackLets[TrackCheck_pre[TemporalMatch[i][j]]].push_back(
                                    std::make_pair(i + 1, j));
        ...
        cout << endl;
        cout << "==============================================" << endl;
        cout << "the number of static feature tracklets: " << TrackLets.size()
             << endl;
        cout << "==============================================" << endl;
        cout << endl;
        ...
        // for (int i = 0; i < N; ++i)
        //     cout << "The length of " << i+2 << " tracklets is found with the amount
        //     of " << TrackLength[i] << " ..." << endl;
        // cout << endl;
    ...
    std::vector<std::vector<std::pair<int, int>>> Tracking::GetDynamicTrackNew() {
        ...
        cout << endl;
        cout << "==============================================" << endl;
        cout << "the number of dynamic feature tracklets: " << TrackLets.size()
             << endl;
        cout << "==============================================" << endl;
        cout << endl;
        ...
        // for (int i = 0; i < N; ++i){
        //     if(TrackLength[i]!=0)
        //         cout << "The length of " << i+2 << " tracklets is found with the
        //         amount of " << TrackLength[i] << " ..." << endl;
        // }
        // cout << endl;
    ...
    Tracking::GetObjTrackTime(std::vector<std::vector<int>> &ObjTrackLab,
                              std::vector<std::vector<int>> &ObjSemanticLab,
        ...
        for (int i = 0; i < ObjTrackLab.size(); ++i) {
            ...
            for (int j = 1; j < ObjTrackLab[i].size(); ++j) {
                // TrackCountGT[ObjTrackLab[i][j]-1] = TrackCountGT[ObjTrackLab[i][j]-1] +
                // 1;
        ...
        //     cout << "Object " << i+1 << " has been tracked " << TrackCount[i] << "
        //     times." << endl;
        // }
    ...
    std::vector<std::vector<std::pair<int, int>>> Tracking::GetDynamicTrack() {
            ...
            if (i == 0) {
                ...
                if (M_pre == 0) {
                    ...
                    for (int k = 1; k < M_pre; k = k + 2) {
                        ...
                        // threshold
                        if (min_dist < 1.0) {
                            // current feature not occupied -or- occupied but new distance is
                            // smaller then label current match
                            if (TM[candi] == -1 ||
                                (TM[candi] != -1 && min_dist < MinDist[candi])) {
                                TM[candi] = k;
                    ...
                    for (int j = 0; j < M_cur; j = j + 2) {
                        // check the TM. if it is associated with last frame, then add to
                        // existing tracklets.
                        if (TM[j] != -1) {
                            TrackLets[TrackCheck_pre[TM[j]]].push_back(
                                    std::make_pair(i, j + 1)); // used to be i+1
        ...
        cout << endl;
        cout << "==============================================" << endl;
        cout << "the number of object feature tracklets: " << TrackLets.size()
             << endl;
        cout << "==============================================" << endl;
        cout << endl;
        ...
        for (int i = 0; i < N; ++i)
            cout << "The length of " << i + 2
                 << " tracklets is found with the amount of " << TrackLength[i]
                 << " ..." << endl;
        cout << endl;
    ...
    void Tracking::RenewFrameInfo(const std::vector<int> &TM_sta) {
        ...
        for (int i = 0; i < TM_sta.size(); ++i) {
            ...
            if (flow_xe != 0 && flow_ye != 0) {
                if (mCurrentFrame.mvStatKeys[TM_sta[i]].pt.x + flow_xe <
                    mImGrayLast.cols &&
                    mCurrentFrame.mvStatKeys[TM_sta[i]].pt.y + flow_ye <
                    mImGrayLast.rows &&
                    mCurrentFrame.mvStatKeys[TM_sta[i]].pt.x + flow_xe > 0 &&
                    mCurrentFrame.mvStatKeys[TM_sta[i]].pt.y + flow_ye > 0) {
                    mvKeysTmp.push_back(mCurrentFrame.mvStatKeys[TM_sta[i]]);
                    mvCorresTmp.push_back(cv::KeyPoint(
                            mCurrentFrame.mvStatKeys[TM_sta[i]].pt.x + flow_xe,
                            mCurrentFrame.mvStatKeys[TM_sta[i]].pt.y + flow_ye, 0, 0, 0, -1));
        ...
        while (tot_num < max_num_sta) {
            ...
            for (int i = start_id; i < mvKeysSample.size(); i = i + step) {
                ...
                for (int j = 0; j < mvKeysTmpCheck.size(); ++j) {
                    float cur_dist =
                            std::sqrt((mvKeysTmpCheck[j].pt.x - mvKeysSample[i].pt.x) *
                                      (mvKeysTmpCheck[j].pt.x - mvKeysSample[i].pt.x) +
                                      (mvKeysTmpCheck[j].pt.y - mvKeysSample[i].pt.y) *
                                      (mvKeysTmpCheck[j].pt.y - mvKeysSample[i].pt.y));
                ...
                if (flow_xe != 0 && flow_ye != 0) {
                    if (mvKeysSample[i].pt.x + flow_xe < mImGrayLast.cols &&
                        mvKeysSample[i].pt.y + flow_ye < mImGrayLast.rows &&
                        mvKeysSample[i].pt.x + flow_xe > 0 &&
                        mvKeysSample[i].pt.y + flow_ye > 0) {
                        mvKeysTmp.push_back(mvKeysSample[i]);
                        mvCorresTmp.push_back(cv::KeyPoint(mvKeysSample[i].pt.x + flow_xe,
                                                           mvKeysSample[i].pt.y + flow_ye, 0,
                                                           0, 0, -1));
                        mvFlowNextTmp.push_back(cv::Point2f(flow_xe, flow_ye));
        ...
        for (int i = 0; i < mCurrentFrame.N_s_tmp; ++i) {
            mv3DPointTmp[i] =
                    Optimizer::Get3DinWorld(mvKeysTmp[i], mvDepthTmp[i], mK,
                                            Converter::toInvMatrix(mCurrentFrame.mTcw));
        }
        ...
        // cout << "updating STATIC features finished...... " << mvKeysTmp.size() <<
        // endl;
        ...
        for (int i = 0; i < ObjInlierSet.size(); ++i) {
            ...
            for (int j = 0; j < ObjInlierSet[i].size(); ++j) {
                ...
                if (mSegMap.at<int>(y, x) != 0 && mDepthMap.at<float>(y, x) < 25 &&
                    mDepthMap.at<float>(y, x) > 0) {
                    ...
                    if (x + flow_x < mImGrayLast.cols && y + flow_y < mImGrayLast.rows &&
                        ...
                        mvObjCorresTmp.push_back(
                                cv::KeyPoint(x + flow_x, y + flow_y, 0, 0, 0, -1));
        ...
        // (2) Save extra key points to make each object having a fixed number (max =
        // 400, 800, 1000)
        ...
        for (int i = 0; i < ObjSet.size(); ++i) {
            ...
            while (tot_num < max_num_obj) {
                // start id > step number, then stop
                if (start_id == step) {
                    // cout << "run on all the original objset... tot_num: " << tot_num <<
                    // endl;
                    break;
                }
                ...
                for (int j = start_id; j < mvTmpSemObjLabel.size(); j = j + step) {
                    ...
                    for (int k = 0; k < mvObjKeysTmpCheck.size(); ++k) {
                        float cur_dist =
                                std::sqrt((mvObjKeysTmpCheck[k].pt.x - mvTmpObjKeys[j].pt.x) *
                                          (mvObjKeysTmpCheck[k].pt.x - mvTmpObjKeys[j].pt.x) +
                                          (mvObjKeysTmpCheck[k].pt.y - mvTmpObjKeys[j].pt.y) *
                                          (mvObjKeysTmpCheck[k].pt.y - mvTmpObjKeys[j].pt.y));
                        if (cur_dist < min_dist)
                            min_dist = cur_dist;
                        if (min_dist < 1.0) {
        ...
        for (int i = 0; i < mCurrentFrame.nSemPosition.size(); ++i) {
            ...
            for (int j = 0; j < UniLab.size(); ++j) {
                if (UniLab[j] == CurSemLabel &&
                    mCurrentFrame.bObjStat[i]) // && mCurrentFrame.bObjStat[i]
                {
                    NewLab[j] = true;
                    break;
                }
        ...
        for (int i = 0; i < mvObjKeysTmp.size(); ++i)
            mvObj3DPointTmp[i] =
                    Optimizer::Get3DinWorld(mvObjKeysTmp[i], mvObjDepthTmp[i], mK,
                                            Converter::toInvMatrix(mCurrentFrame.mTcw));
        ...
        // cout << "updating DYNAMIC features finished...... " << mvObjKeysTmp.size()
        // << endl;
    ...
    void Tracking::UpdateMask() {
                ...
                for (int j = 0; j < mImGrayLast.rows; j++) {
                    for (int k = 0; k < mImGrayLast.cols; k++) {
                        if (mSegMapLast.at<int>(j, k) == UniLab[i]) {
                            ...
                            if (k + flow_x < mImGrayLast.cols && k + flow_x > 0 &&
                                j + flow_y < mImGrayLast.rows && j + flow_y > 0)
        ...
        //                 break;
        //             case 4:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(47,255,173); //
        //                 greenyellow break;
        //             case 5:
        ...
        //                 break;
        //             case 10:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(245,245,245); //
        //                 whitesmoke break;
        //             case 11:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(0,165,255); //
        //                 orange break;
        //             case 12:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(230,216,173); //
        //                 lightblue break;
        //             case 13:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(128,128,128); //
        //                 grey break;
        //             case 14:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(0,215,255); // gold
        //                 break;
        //             case 15:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(30,105,210); //
        //                 chocolate break;
        //             case 16:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(0,255,0);  // green
        //                 break;
        //             case 17:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(34, 34, 178);  //
        //                 firebrick break;
        //             case 18:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(240, 255, 240);  //
        //                 honeydew break;
        //             case 19:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(250, 206, 135);  //
        //                 lightskyblue break;
        //             case 20:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(238, 104, 123);  //
        //                 mediumslateblue break;
        //             case 21:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(225, 228, 255);  //
        //                 mistyrose break;
        //             case 22:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(128, 0, 0);  //
        //                 navy break;
        //             case 23:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(35, 142, 107);  //
        //                 olivedrab break;
        //             case 24:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(45, 82, 160);  //
        //                 sienna break;
        //             case 25:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(0, 255, 127); //
        //                 chartreuse break;
        //             case 26:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(139, 0, 0);  //
        //                 darkblue break;
        //             case 27:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(60, 20, 220);  //
        //                 crimson break;
        //             case 28:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(0, 0, 139);  //
        //                 darkred break;
        //             case 29:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(211, 0, 148);  //
        //                 darkviolet break;
        //             case 30:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(255, 144, 30);  //
        //                 dodgerblue break;
        //             case 31:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(105, 105, 105);  //
        //                 dimgray break;
        //             case 32:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(180, 105, 255);  //
        //                 hotpink break;
        //             case 33:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(204, 209, 72);  //
        //                 mediumturquoise break;
        //             case 34:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(173, 222, 255);  //
        //                 navajowhite break;
        //             case 35:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(143, 143, 188); //
        //                 rosybrown break;
        //             case 36:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(50, 205, 50);  //
        //                 limegreen break;
        //             case 37:
        //                 imgLabel.at<cv::Vec3b>(i,j) = cv::Vec3b(34, 34, 178);  //
        //                 firebrick break;
    ...
    void Tracking::GetMetricError(
            const std::vector<cv::Mat> &CamPose,
            const std::vector<std::vector<cv::Mat>> &RigMot,
        ...
        for (int i = 1; i < CamPose.size(); ++i) {
            ...
            // translation
            float t_ate_cam =
                    std::sqrt(ate_cam.at<float>(0, 3) * ate_cam.at<float>(0, 3) +
                              ate_cam.at<float>(1, 3) * ate_cam.at<float>(1, 3) +
                              ate_cam.at<float>(2, 3) * ate_cam.at<float>(2, 3));
            if (bRMSError)
        ...
        cout << "average error (Camera):"
             << " t: " << t_sum << " R: " << r_sum << endl;
        ...
        for (int i = 0; i < RigMot.size(); ++i) {
            if (RigMot[i].size() > 1) {
                for (int j = 1; j < RigMot[i].size(); ++j) {
                    if (!ObjStat[i][j]) {
                        cout << "(" << mpMap->vnRMLabel[i][j] << ")"
                             << " is a failure case." << endl;
                        continue;
                    }

                    cv::Mat RigMotBody = Converter::toInvMatrix(ObjPosePre[i][j]) *
                                         RigMot[i][j] * ObjPosePre[i][j];
                    cv::Mat rpe_obj = Converter::toInvMatrix(RigMotBody) * RigMot_gt[i][j];

                    // translation error
                    float t_rpe_obj =
                            std::sqrt(rpe_obj.at<float>(0, 3) * rpe_obj.at<float>(0, 3) +
                                      rpe_obj.at<float>(1, 3) * rpe_obj.at<float>(1, 3) +
                                      rpe_obj.at<float>(2, 3) * rpe_obj.at<float>(2, 3));
                    if (bRMSError) {
                        each_obj_t[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_t[mpMap->vnRMLabel[i][j] - 1] + t_rpe_obj * t_rpe_obj;
                        t_rpe_sum = t_rpe_sum + t_rpe_obj * t_rpe_obj;
                    } else {
                        each_obj_t[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_t[mpMap->vnRMLabel[i][j] - 1] + t_rpe_obj;
                    ...
                    if (bRMSError) {
                        each_obj_r[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_r[mpMap->vnRMLabel[i][j] - 1] + r_rpe_obj * r_rpe_obj;
                        r_rpe_sum = r_rpe_sum + r_rpe_obj * r_rpe_obj;
                    } else {
                        each_obj_r[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_r[mpMap->vnRMLabel[i][j] - 1] + r_rpe_obj;
                        r_rpe_sum = r_rpe_sum + r_rpe_obj;
                    }

                    // cout << "(" << mpMap->vnRMLabel[i][j] << ")" << " t: " << t_rpe_obj
                    // << " R: " << r_rpe_obj << endl;
                    obj_count++;
                    each_obj_count[mpMap->vnRMLabel[i][j] - 1] =
                            each_obj_count[mpMap->vnRMLabel[i][j] - 1] + 1;
                }
            }
        }
        if (bRMSError) {
        ...
        cout << "average error (Over All Objects):"
             << " t: " << t_rpe_sum << " R: " << r_rpe_sum << endl;
        ...
        for (int i = 0; i < each_obj_count.size(); ++i) {
            ...
            if (each_obj_count[i] >= 3)
                cout << endl
                     << "average error of Object " << i + 1 << ": "
                     << " t: " << each_obj_t[i] << " R: " << each_obj_r[i]
                     << " TrackCount: " << each_obj_count[i] << endl;
        }

        cout << "=================================================" << endl;
    }

    void Tracking::PlotMetricError(
            const std::vector<cv::Mat> &CamPose,
            const std::vector<std::vector<cv::Mat>> &RigMot,
            const std::vector<std::vector<cv::Mat>> &ObjPosePre,
            const std::vector<cv::Mat> &CamPose_gt,
            ...
            if (RigMot[i].size() > 1) {
                for (int j = 1; j < RigMot[i].size(); ++j) {
                    ...
                    each_obj_count[mpMap->vnRMLabel[i][j] - 1] =
                            each_obj_count[mpMap->vnRMLabel[i][j] - 1] + 1;
                    if (bAccumError) {
                        ObjTraErr[mpMap->vnRMLabel[i][j] - 1].push_back(
                                each_obj_t[mpMap->vnRMLabel[i][j] - 1] /
                                each_obj_count[mpMap->vnRMLabel[i][j] - 1]);
                        ObjRotErr[mpMap->vnRMLabel[i][j] - 1].push_back(
                                each_obj_r[mpMap->vnRMLabel[i][j] - 1] /
                                each_obj_count[mpMap->vnRMLabel[i][j] - 1]);
                    } else {
        ...
        for (int i = 0; i < each_obj_count.size(); ++i) {
            ...
            if (each_obj_count[i] >= 3)
                cout << endl
                     << "average error of Object " << i + 1 << ": "
                     << " t: " << each_obj_t[i] << " R: " << each_obj_r[i] << endl;
        }
    ...
    void Tracking::GetVelocityError(
            const std::vector<std::vector<cv::Mat>> &RigMot,
            const std::vector<std::vector<cv::Mat>> &PointDyn,
            const std::vector<std::vector<int>> &FeaLab,
            const std::vector<std::vector<int>> &RMLab,
        ...
        for (int i = 0; i < RigMot.size(); ++i) {
            ...
            // Check if there are moving objects, and if all the variables are
            // consistent
            if (RigMot[i].size() > 1 && Velo_gt[i].size() > 1 && RMLab[i].size() > 1) {
                ...
                for (int j = 1; j < RigMot[i].size(); ++j) {
                    ...
                    cv::Mat sp_est_v = RigMot[i][j].rowRange(0, 3).col(3) -
                                       (cv::Mat::eye(3, 3, CV_32F) -
                                        RigMot[i][j].rowRange(0, 3).colRange(0, 3)) *
                                       ObjCenter;
                    float sp_est_norm =
                            std::sqrt(sp_est_v.at<float>(0) * sp_est_v.at<float>(0) +
                                      sp_est_v.at<float>(1) * sp_est_v.at<float>(1) +
                    ...
                    if (bRMSError) {
                        each_obj_est[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_est[mpMap->vnRMLabel[i][j] - 1] +
                                sp_est_norm * sp_est_norm;
                        each_obj_gt[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_gt[mpMap->vnRMLabel[i][j] - 1] +
                                Velo_gt[i][j] * Velo_gt[i][j];
                        s_sum = s_sum + speed_error * speed_error;
                    } else {
                        each_obj_est[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_est[mpMap->vnRMLabel[i][j] - 1] + sp_est_norm;
                        each_obj_gt[mpMap->vnRMLabel[i][j] - 1] =
                                each_obj_gt[mpMap->vnRMLabel[i][j] - 1] + Velo_gt[i][j];
                    ...
                    // cout << "(" << i+1 << "/" << mpMap->vnRMLabel[i][j] << ")" << " s: "
                    // << speed_error << " est: " << sp_est_norm << " gt: " << Velo_gt[i][j]
                    // << endl;
        ...
        cout << "average speed error (All Objects):"
             << " s: " << s_sum << "km/h "
             << "Track Num: " << (int) obj_count << " GT AVG SPEED: " << s_gt_sum
             << endl;
        ...
        for (int i = 0; i < each_obj_count.size(); ++i) {
            ...
            if (mpMap->nObjTraCount[i] >= 3)
                cout << endl
                     << "average error of Object " << i + 1 << " ("
                     << mpMap->nObjTraCount[i] << "/" << mpMap->nObjTraCountGT[i] << "/"
                     << mpMap->nObjTraSemLab[i] << "): "
                     << " (est) " << each_obj_est[i] << " (gt) " << each_obj_gt[i]
                     << endl;
        }
